...
# test script to capture vectors and draw them

from gimpfu import *

def get_coords_by_name(image,layer_name):
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,layer_name))
    fluf,x1,y1,x2,y2=pdb.gimp_selection_bounds(image)
...
def draw_tummy(image):
    ...
    pdb.gimp_edit_blend(patch_layer,
            ...
            True,               # Dither
            x_pos,y1,           # Blend X,Y Startpoint
            x_pos,y2)           # Blend X,Y Endpoint
    pdb.gimp_selection_none(image)
...
def draw_left_foot(image):
    ...
    pdb.gimp_edit_blend(left_foot_glow_layer,
            ...
            0,                  # Supersampling Threshold
            True,               # Dither
            x1+7,y_pos,         # Blend X,Y Startpoint
            x2-7,y_pos)         # Blend X,Y Endpoint
    pdb.gimp_selection_none(image)
...
def draw_right_eyelid(image):
    ...
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Right Eyelid"))
    x1,y1,x2,y2=get_coords_by_name(image,"Right Eyelid")
    ...
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Right Eyelid"))
...
def draw_left_eyelid(image):
    ...
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Left Eyelid"))
    x1,y1,x2,y2=get_coords_by_name(image,"Left Eyelid")
    ...
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Left Eyelid"))
    pdb.gimp_selection_shrink(image, 2)
    x_pos=x2-((x2-x1)/2)
    y_pos=y2-((y2-y1)/2)
...
def draw_right_eye(image):
    ...
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Right Eye Glow"))
    ...
    x1,y1,x2,y2=get_coords_by_name(image,"Right Eye")
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Right Eye"))
    pdb.gimp_selection_shrink(image, 2)
    x_pos=x2-((x2-x1)/2)
    y_pos=y2-((y2-y1)/2)
...
def draw_left_eye(image):
    ...
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Left Eye Glow"))
    ...
    x1,y1,x2,y2=get_coords_by_name(image,"Left Eye")
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Left Eye"))
    pdb.gimp_selection_shrink(image, 2)
    x_pos=x2-((x2-x1)/2)
    y_pos=y2-((y2-y1)/2)
...
def draw_right_pupil(image):
    ...
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Right Pupil"))
    ...
    x1,y1,x2,y2=get_coords_by_name(image,"Right Pupil")
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Right Pupil"))
    ...
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Right Pupil"))
    right_pupil_reflection_layer_bottom=gimp.Layer(image, "Right Pupil Reflection Bottom", 256, 256, 
            RGBA_IMAGE, 60, NORMAL_MODE)
...
def draw_left_pupil(image):
    ...
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Left Pupil"))
    left_pupil_reflection_layer_top=gimp.Layer(image, "Left Pupil Reflection Top", 256, 256, 
            RGBA_IMAGE, 100, NORMAL_MODE)
    image.add_layer(left_pupil_reflection_layer_top,layer_pos)
    left_pupil_reflection_layer_top.fill(TRANSPARENT_FILL)
    x1,y1,x2,y2=get_coords_by_name(image,"Left Pupil")
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Left Pupil"))
    pdb.gimp_selection_shrink(image, 1)
    x_pos=x2-((x2-x1)/2)
    ...
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Left Pupil"))
    left_pupil_reflection_layer_bottom=gimp.Layer(image, "Left Pupil Reflection Bottom", 256, 256, 
            RGBA_IMAGE, 60, NORMAL_MODE)
...
def draw_beak(image):
    ...
    pdb.gimp_edit_blend(beak_layer,
            ...
            0,                  # Supersampling Threshold
            True,               # Dither
            x1,y1,              # Blend X,Y Startpoint
            x2,y1)        # Blend X,Y Endpoint
    pdb.gimp_selection_shrink(image, 7)
    fluf,x1,y1,x2,y2=pdb.gimp_selection_bounds(image)
    ...
    pdb.gimp_edit_blend(beak_glow_layer,
            ...
            0,                  # Supersampling Threshold
            True,               # Dither
            x1,y1+4,            # Blend X,Y Startpoint
            x2,y1+4)            # Blend X,Y Endpoint
    pdb.gimp_selection_none(image)
...
def add_body_reflections(image):
    ...
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Body"))
    ...
    x1,y1,x2,y2=get_coords_by_name(image,"Right Eye")
    pdb.gimp_ellipse_select(image,x1-59,y1-47,119,103,CHANNEL_OP_ADD,False,False,1)
    fluf,x1,y1,x2,y2=pdb.gimp_selection_bounds(image)
    x_pos=x2-((x2-x1)/2)
    pdb.gimp_edit_blend(body_reflection_layer_top,
            ...
            0,                  # Supersampling Threshold
            True,               # Dither
            x_pos,y1+8,         # Blend X,Y Startpoint
            x_pos,y2)           # Blend X,Y Endpoint
    pdb.gimp_selection_none(image)
    pdb.plug_in_gauss(image, body_reflection_layer_top, 2.0, 2.0, 1)
    
    body_reflection_layer_bottom=gimp.Layer(image, "Body Reflection Bottom", 256, 256,
            RGBA_IMAGE, 60, NORMAL_MODE)
    image.add_layer(body_reflection_layer_bottom,layer_pos+1)
    body_reflection_layer_bottom.fill(TRANSPARENT_FILL)
    beak_x1,beak_y1,beak_x2,beak_y2=get_coords_by_name(image,"Beak")
    x1,y1,x2,y2=get_coords_by_name(image,"Body")
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Body"))
    pdb.gimp_selection_shrink(image, 2)
    x_pos=x2-((x2-x1)/2)
    pdb.gimp_edit_blend(body_reflection_layer_bottom,
            ...
            True,               # Dither
            x_pos,y2,           # Blend X,Y Startpoint
            x_pos,beak_y2)      # Blend X,Y Endpoint
    pdb.gimp_selection_none(image)
    x1,y1,x2,y2=get_coords_by_name(image,"White Patch")
    pdb.gimp_ellipse_select(image,x1-30,y1-45,200,160,CHANNEL_OP_ADD,False,False,1)
    pdb.gimp_edit_cut(body_reflection_layer_bottom)
    pdb.gimp_selection_none(image)

    # Beak Reflection Top
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Beak Glow"))
    ...
    x1,y1,x2,y2=get_coords_by_name(image,"Beak")
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Beak"))
    pdb.gimp_selection_shrink(image, 2)
    ...
    pdb.gimp_edit_blend(beak_reflection_layer_top,
            ...
            0,                  # Supersampling Threshold
            True,               # Dither
            x2,y1,              # Blend X,Y Startpoint
            x1,y2)              # Blend X,Y Endpoint
    pdb.gimp_selection_none(image)
    x1,y1,x2,y2=get_coords_by_name(image, "Right Eye")
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Right Eye"))
    ...
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"White Patch"))
    white_patch_reflection_layer_top=gimp.Layer(image, "White Patch Reflection Top", 256, 256,
            RGBA_IMAGE, 50, NORMAL_MODE)
    image.add_layer(white_patch_reflection_layer_top,layer_pos)
    white_patch_reflection_layer_top.fill(TRANSPARENT_FILL)
    x1,foot_y1,x2,y2=get_coords_by_name(image,"Left Foot")
    x1,y1,x2,y2=get_coords_by_name(image,"White Patch")
    x_pos=x2-((x2-x1)/2)
    pdb.gimp_ellipse_select(image,x1+5,y1-19,127,165,CHANNEL_OP_ADD,False,False,1)
    pdb.gimp_edit_blend(white_patch_reflection_layer_top,
            ...
            True,               # Dither
            x_pos,y1,           # Blend X,Y Startpoint
            x_pos,foot_y1)      # Blend X,Y Endpoint
    pdb.gimp_selection_none(image)
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"White Patch"))
    pdb.gimp_selection_invert(image)
    ...
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"White Patch"))
    white_patch_reflection_layer_bottom=gimp.Layer(image, "White Patch Reflection Bottom", 256, 256,
            RGBA_IMAGE, 40, NORMAL_MODE)
    ...
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Left Foot Glow"))
    left_foot_reflection_layer=gimp.Layer(image, "Left Foot Reflection", 256, 256,
            RGBA_IMAGE, 100, NORMAL_MODE)
    ...
    x_pos=y2-((y2-y1)/2)
    y_pos=x2-((x2-x1)/2)
    pdb.gimp_ellipse_select(image,x1-41,y1+152,width,height,CHANNEL_OP_ADD,False,False,1)
    ...
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Right Foot Glow"))
    ...
    pdb.gimp_item_transform_flip(right_foot_reflection_layer, image.width/2,0,image.width/2,image.height)

    # Left Wing Reflection Top
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Left Wing"))
    x1,y1,x2,y2=get_coords_by_name(image,"Left Wing")
    left_wing_reflection_layer_top=gimp.Layer(image, "Left Wing Reflection Top", 256, 256,
            RGBA_IMAGE, 70, NORMAL_MODE)
    image.add_layer(left_wing_reflection_layer_top,layer_pos)
    left_wing_reflection_layer_top.fill(TRANSPARENT_FILL)
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Left Wing"))
    pdb.gimp_selection_shrink(image, 2)
    pdb.gimp_edit_blend(left_wing_reflection_layer_top,
            ...
            0,                  # Supersampling Threshold
            True,               # Dither
            x2-(x2-x1)/2,       # Blend X Startpoint
            y2-(y2-y1)/2,       # Blend Y Startpoint
            x2,y2)              # Blend X,Y Endpoint
    ...
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Right Wing"))
    ...
    pdb.gimp_item_transform_flip(right_wing_reflection_layer_top, image.width/2,0,image.width/2,image.height)

    # Left Wing Reflection Bottom
    pdb.gimp_context_set_foreground((255, 255, 255))
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Left Wing"))
    left_wing_reflection_layer_bottom=gimp.Layer(image, "Left Wing Reflection Bottom", 256, 256,
            RGBA_IMAGE, 70, NORMAL_MODE)
    image.add_layer(left_wing_reflection_layer_bottom,layer_pos)
    left_wing_reflection_layer_bottom.fill(TRANSPARENT_FILL)
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Left Wing"))
    pdb.gimp_selection_shrink(image, 1)
    pdb.gimp_edit_bucket_fill(left_wing_reflection_layer_bottom, FG_BUCKET_FILL,NORMAL_MODE,100, 255, False, x1+2,y1+2)
    pdb.gimp_selection_shrink(image, 2)
    pdb.gimp_edit_cut(left_wing_reflection_layer_bottom)
    x1,y1,x2,y2=get_coords_by_name(image,"Right Eye")
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Right Eye"))
    pdb.gimp_selection_translate(image,x1-125,y1+36)
    pdb.gimp_selection_invert(image)
    pdb.gimp_edit_cut(left_wing_reflection_layer_bottom)
    pdb.gimp_selection_none(image)
    ...
    pdb.gimp_item_transform_flip(right_wing_reflection_layer_bottom, image.width/2,0,image.width/2,image.height)
...
def add_shadows(image):
    ...
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Left Foot"))
    x1,y1,x2,y2=get_coords_by_name(image,"Left Foot")
    left_foot_body_shadow_layer=gimp.Layer(image, "Left Foot Body Shadow", 256, 256,
            RGBA_IMAGE, 30, NORMAL_MODE)
    image.add_layer(left_foot_body_shadow_layer,layer_pos+1)
    left_foot_body_shadow_layer.fill(TRANSPARENT_FILL)
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Left Foot"))
    pdb.gimp_selection_translate(image,10,-1)
    pdb.gimp_edit_bucket_fill(left_foot_body_shadow_layer, FG_BUCKET_FILL, NORMAL_MODE,100, 255, False, x1+2,y1+2)
    pdb.gimp_selection_none(image)
    ...
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Right Foot"))
    ...
    pdb.gimp_item_transform_flip(right_foot_body_shadow_layer, image.width/2,0,image.width/2,image.height)

    # Left On Foot Shadow
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Left Foot Reflection"))
    left_on_foot_shadow_layer=gimp.Layer(image, "On Left Foot Shadow", 256, 256,
            RGBA_IMAGE, 20, NORMAL_MODE)
    image.add_layer(left_on_foot_shadow_layer,layer_pos)
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Left Foot"))
    pdb.gimp_selection_grow(image, 5)
    fluf,x1,y1,x2,y2=pdb.gimp_selection_bounds(image)
    pdb.gimp_edit_bucket_fill(left_on_foot_shadow_layer, FG_BUCKET_FILL, NORMAL_MODE,100, 255, False, x1+2,y1+2)
    ...
    layer_pos=pdb.gimp_image_get_layer_position(image,pdb.gimp_image_get_layer_by_name(image,"Right Foot Reflection"))
    ...
    pdb.gimp_item_transform_flip(right_on_foot_shadow_layer, image.width/2,0,image.width/2,image.height)

    # Beak Drop Shadow
    pdb.script_fu_drop_shadow(image,pdb.gimp_image_get_layer_by_name(image,"Beak"),2, 8, 15, (0,0,0), 63, False)
    ...
    pdb.script_fu_drop_shadow(image,pdb.gimp_image_get_layer_by_name(image,"Left Eye"),4, 4, 15, (0,0,0), 63, False)
    shadow_layer=pdb.gimp_image_get_layer_by_name(image,"Drop Shadow")
    ...
    pdb.script_fu_drop_shadow(image,pdb.gimp_image_get_layer_by_name(image,"Right Eye"),4, 4, 15, (0,0,0), 63, False)
    shadow_layer=pdb.gimp_image_get_layer_by_name(image,"Drop Shadow")
    ...
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Body"))
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Left Foot"))
    pdb.gimp_image_select_item(image,CHANNEL_OP_ADD, pdb.gimp_image_get_layer_by_name(image,"Right Foot"))
    pdb.script_fu_drop_shadow(image,pdb.gimp_image_get_layer_by_name(image,"Background"),2, 8, 15, (0,0,0), 63, False)
    pdb.gimp_selection_none(image)
